...
save_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/S1-seq/LSY4753-3B_4754-3A"
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/01_S1-seq_edit_distance_distributions/LSY4753-3B_4754-3A"

# initialize data.frames for result recording
mapping_stats <- data.frame()

# iterate through samples
samples <- c("LSY4753-3B_2", "LSY4754-3A_2")
sample <- samples[1]
for(sample in samples){
  ...
  # move wrongly assigned coverage at duplicated region (identified by wrong strand) to correct locus (hmr -> MAT)
  duplication_hmr <- GRanges(seqnames = "chrIII", ranges = IRanges(start = 294483, end = 294617), strand = "+")
  # subsetByOverlaps(x = tmp, ranges = duplication_hmr)
  duplication_MAT <- GRanges(seqnames = "chrIII", ranges = IRanges(start = 200852, end = 200986), strand = "+")
  # subsetByOverlaps(x = tmp, ranges = duplication_MAT)
  hits <- findOverlaps(query = duplication_hmr, subject = tmp)
  start(tmp[subjectHits(hits)]) <- start(tmp[subjectHits(hits)]) + (start(duplication_MAT) - start(duplication_hmr))
  end(tmp[subjectHits(hits)]) <- end(tmp[subjectHits(hits)]) + (start(duplication_MAT) - start(duplication_hmr))
  # subsetByOverlaps(x = tmp, ranges = duplication_hmr)
  # subsetByOverlaps(x = tmp, ranges = duplication_MAT)
...
# save data ---------------------------------------------------------------
save(list = paste0(dash_to_underscore(samples), "_S1_seq"), file = paste0(save_dir, "/LSY4753-3B_4754-3A.RData"))
save(list = paste0(dash_to_underscore(samples), "_S1_seq_unnormalized"), file = paste0(save_dir, "/LSY4753-3B_4754-3A_unnormalized.RData"))
save(list = paste0(dash_to_underscore(samples), "_S1_seq_w_chrM_and_2micron"), file = paste0(save_dir, "/LSY4753-3B_4754-3A_w_chrM_and_2micron.RData"))
