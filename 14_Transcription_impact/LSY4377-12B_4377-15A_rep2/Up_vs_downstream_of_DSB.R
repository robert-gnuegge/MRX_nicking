...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/11_Transcription_impact/LSY4377-12B_4377-15A_rep2/Up_vs_downstream_of_DSB"
...
par(cex = 1, mar = c(5.1, 4.1, 4.1, 2.1) - c(1.5, 0.5, 3.7, 1.7), tcl = -0.3, mgp = c(2.5, 0.6, 0), las = 1)
  ...
  f <- 0.25
  y <- trans(x = y, threshold = th, threshold_trans = th_t, factor = f)
  plot(x = x, y = y, ylim = c(0, trans(x = 165, threshold = th, threshold_trans = th_t, factor = f)), 
       xlim = c(0, max(agg$distance_to_DSB[agg$prom_distal])), ylab = "Average S1-seq [RPM]", xlab = "Distance from DSB [nt]", 
       type = "l", lwd = 1.5, col = JFly_colors[1], yaxt = "n", xaxt = "n")
  axis(side = 1, at = c(0, 2, 4, 6) * 100)
  ...
  below_break <- c(0, 5, 10, 15)
  above_break <- c(140, 150, 160)
  axis(side = 2, at = c(below_break, trans(x = above_break, threshold = th, threshold_trans = th_t, factor = f)), labels = c(below_break, above_break))
