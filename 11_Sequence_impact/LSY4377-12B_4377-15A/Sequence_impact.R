...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/08_Sequence_impact/LSY4377-12B_4377-15A"
...
#         A        T         G         C
# 0.2933365 0.307138 0.1945814 0.2049434
...
pdf(file = "tmp.pdf", width = 7.5, height = 3.5)
par(cex = 1, mar = c(5.1, 4.1, 4.1, 2.1) - c(1.5, 0.5, 1, 1.7), tcl = -0.25, mgp = c(2.25, 0.5, 0), las = 1)

# A
plot(x = 1:dim(PWM)[2], y = PWM["A", ], type = "l", col = my_colors["A"], ylim = range(c(PWM, PWM_bg, 0.55)),
     xlab = "Distance from Nick Site [nt]", ylab = "Fraction", xaxt = "n")
...
axis(side = 1, at = c(0:6 * 5 + 1, 35, 36, 8:14 * 5), labels = NA, tcl = -0.3)
axis(side = 1, at = c(0:3 * 10 + 1, 4:7 * 10), labels = c(-3:0 * 10 - 5, 0:3 * 10 + 5), tick = FALSE)
...
# add legend
text(x = 68, y = c(0.305, 0.305, 0.1925, 0.1925), pos = c(3, 1, 1, 3), labels = c("T", "A", "G", "C"), col = JFly_colors[c(2, 1, 3, 8)])

# add most frequent nts
most_freq <- rownames(PWM)[apply(X = PWM, MARGIN = 2, FUN = which.max)]
idx <- 1:70
text(x = idx, y = 0.542, labels = most_freq[idx], col = my_colors[most_freq[idx]], cex = 0.75)
# underline
y <- 0.525
x_expand <- 0.4
segments(x0 = c(19, 25, 37, 49) - x_expand, y0 = y, x1 = c(23, 35, 47, 53) + x_expand, y1 = y, col = "gray")
segments(x0 = c(9, 59) - x_expand, y0 = y, x1 = c(13, 63) + x_expand, y1 = y, col = "gray", lty = "dotted")
