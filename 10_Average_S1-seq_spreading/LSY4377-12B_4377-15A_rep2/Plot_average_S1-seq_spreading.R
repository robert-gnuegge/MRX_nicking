...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/07_Average_S1-seq_spreading/LSY4377-12B_4377-15A_rep2"
...
LSY4377_12B_0 <- subsetByIntersect(subject = LSY4377_12B_0_rep2_S1_seq, query = DSB_regions(DSBs = SrfIcs[-c(9, 17)], region_width = 5000, up_rev_down_fw = TRUE))
LSY4377_12B_1 <- subsetByIntersect(subject = LSY4377_12B_1_rep2_S1_seq, query = DSB_regions(DSBs = SrfIcs[-c(9, 17)], region_width = 5000, up_rev_down_fw = TRUE))
LSY4377_12B_2 <- subsetByIntersect(subject = LSY4377_12B_2_rep2_S1_seq, query = DSB_regions(DSBs = SrfIcs[-c(9, 17)], region_width = 5000, up_rev_down_fw = TRUE))
LSY4377_12B_4 <- subsetByIntersect(subject = LSY4377_12B_4_rep2_S1_seq, query = DSB_regions(DSBs = SrfIcs[-c(9, 17)], region_width = 5000, up_rev_down_fw = TRUE))
LSY4377_15A_4 <- subsetByIntersect(subject = LSY4377_15A_4_rep2_S1_seq, query = DSB_regions(DSBs = SrfIcs[-c(9, 17)], region_width = 5000, up_rev_down_fw = TRUE))
...
par(cex = 1, mar = c(5.1, 4.1, 4.1, 2.1) - c(1.5, 0.5, 3.7, 1.7), tcl = -0.3, mgp = c(2.5, 0.6, 0), las = 1)
  ...
  f <- 1/10
  y <- trans(x = y, threshold = th, threshold_trans = th_t, factor = f)
  plot(x = xx, y = y, ylim = c(0, trans(x = 225, threshold = th, threshold_trans = th_t, factor = f)), 
       xlim = c(0, 1800), ylab = "Average S1-seq [RPM]", xlab = "Distance from DSB [nt]", 
       type = "l", lwd = 1.5, col = JFly_colors[1], yaxt = "n")
  ...
  below_break <- 0:4 * 5
  above_break <- c(100, 150, 200)
  axis(side = 2, at = c(below_break, trans(x = above_break, threshold = th, threshold_trans = th_t, factor = f)), labels = c(below_break, above_break))
  
  axis.break(axis = 2, breakpos = th, style = "slash", bgcol = "white")
  rect(xleft = -15, ybottom = th - 0.5, xright = 10, ytop =  th + 0.5, col = "white", border = "white")
...
par(cex = 1, mar = c(5.1, 4.1, 4.1, 2.1) - c(1.5, 0, 3.7, 1.7), tcl = -0.3, mgp = c(2.5, 0.6, 0), las = 1)
  ...
  f <- 1/1000
  y <- trans(x = y, threshold = th, threshold_trans = th_t, factor = f)
  plot(x = x, y = y, ylim = c(0, trans(x = 12000, threshold = th, threshold_trans = th_t, factor = f)),
       xlim = c(0, 1800), ylab = NA, xlab = "Distance from DSB [nt]",
       type = "l", lwd = 1.5, col = JFly_colors[1], yaxt = "n")
  title(ylab = "Average S1-seq [RPM]", line = 3)
  below_break <- 0:5 * 2
  above_break <- c(10000, 12000)
  axis(side = 2, at = c(below_break, trans(x = above_break, threshold = th, threshold_trans = th_t, factor = f)), labels = c(below_break, above_break))
  axis.break(axis = 2, breakpos = th, style = "slash", bgcol = "white")
