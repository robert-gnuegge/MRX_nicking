...
LSY4602_20C_1_S1_seq <- subsetByIntersect(subject = LSY4602_20C_1_S1_seq, query = DSB_regions(DSBs = SrfIcs, region_width = 10000))
...
LSY4602_20C_1_smooth <- as_smoothed_GRanges(GRanges = LSY4602_20C_1, hanning_window_size = n)
LSY4377_12B_1_smooth <- as_smoothed_GRanges(GRanges = LSY4377_12B_1, hanning_window_size = n)


# plotting ================================================================

plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/04_S1-seq_next_to_DSBs/LSY4602-20C_1"
...
for (n in 1:length(SrfIcs)){
  ...
  LSY4602_20C_1_roi <- subsetByOverlaps(x = LSY4602_20C_1, ranges = roi)
  LSY4377_12B_1_roi <- subsetByOverlaps(x = LSY4377_12B_1, ranges = roi)
  
  LSY4602_20C_1_smooth_roi <- subsetByOverlaps(x = LSY4602_20C_1_smooth, ranges = roi)
  LSY4377_12B_1_smooth_roi <- subsetByOverlaps(x = LSY4377_12B_1_smooth, ranges = roi)
  
  # find common ylim
  ylim <- range(c(LSY4602_20C_1_roi, LSY4377_12B_1_roi)$score)
  ylim <- c(min(ylim) - 0.05 * max(pretty(ylim)), max(pretty(ylim)))  # adjust for prettier plotting
  
  ylim_smooth <- range(c(LSY4602_20C_1_smooth_roi, LSY4377_12B_1_smooth_roi)$score)
  ...
  # make OverlayTracks
  DT_LSY4602_20C_1 <- DataTrack_raw_smooth_fw_rev(GRanges_raw = LSY4602_20C_1_roi, GRanges_smooth = LSY4602_20C_1_smooth_roi,
                                                  fw_col = fw_col, rev_col = rev_col, ylim_raw = ylim, ylim_smooth = ylim_smooth, name = "ku70Delta\n1 h")
  DT_LSY4377_12B_1 <- DataTrack_raw_smooth_fw_rev(GRanges_raw = LSY4377_12B_1_roi, GRanges_smooth = LSY4377_12B_1_smooth_roi,
                                                  fw_col = fw_col, rev_col = rev_col, ylim_raw = ylim, ylim_smooth = ylim_smooth, name = "KU70\n1 h")
  
  # plot
  pdf(file = "tmp.pdf", width = 3.5, height = 2)
    plotTracks(trackList = list(DT_LSY4602_20C_1, DT_LSY4377_12B_1),
