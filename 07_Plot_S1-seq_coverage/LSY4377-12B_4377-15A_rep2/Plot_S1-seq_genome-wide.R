...
load(file = "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/S1-seq/LSY4377-12B_4377-15A_rep2/LSY4377-12B_LSY4377-15A_rep2_w_chrM_and_2micron.RData")
...
LSY4377_12B_0 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY4377_12B_0_rep2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
LSY4377_12B_1 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY4377_12B_1_rep2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
LSY4377_12B_2 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY4377_12B_2_rep2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
LSY4377_12B_4 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY4377_12B_4_rep2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
LSY4377_15A_4 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY4377_15A_4_rep2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
...
for (sample in c(paste0("LSY4377_12B_", c(0, 1, 2, 4)), "LSY4377_15A_4")){
  dir.create(paste0(plot_dir, "/", sample, "_rep2"), recursive = TRUE, showWarnings = FALSE)  # create sub directory for each sample
  for (chr in SacCer_chromosomes_df$chr){
    ...
    dev.off()
    GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/", sample, "_rep2/", sample, "_rep2_", chr, ".pdf"))
  }
}