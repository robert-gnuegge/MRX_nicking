...
load(file = "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/S1-seq/LSY5023-98C_5038-9C/LSY5023-98C_5038-9C_w_chrM_and_2micron.RData")
...
LSY5023_98C_2 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY5023_98C_2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)
LSY5038_9C_2 <- sum_scores_in_bins(GRanges = as_GRanges_with_fw_rev_scores(GRanges = LSY5038_9C_2_S1_seq_w_chrM_and_2micron, rev_negative = TRUE), bins = bins)

# find y range that covers all sample score ranges ------------------------
y_range <- range(c(LSY5023_98C_2$fw, LSY5023_98C_2$rev, LSY5038_9C_2$fw, LSY5038_9C_2$rev))
y_range <- c(-max(abs(pretty(y_range))), max(abs(pretty(y_range))))  # makes y_range "nicer"
y_range <- c(-10000, 10000)  # the calculated y_range is larger, but this is only due to a high TEL coverage
...
LSY5023_98C_2_DT <- DataTrack(LSY5023_98C_2, ylim = y_range)
LSY5038_9C_2_DT <- DataTrack(LSY5038_9C_2, ylim = y_range)
...
for (sample in c("LSY5023_98C_2", "LSY5038_9C_2")){
