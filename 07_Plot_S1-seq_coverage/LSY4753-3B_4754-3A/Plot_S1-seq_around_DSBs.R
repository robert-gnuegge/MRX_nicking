...
DSBs <- GRanges(c("chrIII:200753", "chrIII:295660"))
...
LSY4753_3B_2_S1_seq <- subsetByIntersect(subject = LSY4753_3B_2_S1_seq, query = DSB_regions(DSBs = DSBs, region_width = 10000))
LSY4754_3A_2_S1_seq  <- subsetByIntersect(subject = LSY4754_3A_2_S1_seq, query = DSB_regions(DSBs = DSBs, region_width = 10000))

# make nt-resolved GRanges objects and sort
LSY4753_3B_2 <- sort(as_nt_resolved_GRanges(LSY4753_3B_2_S1_seq), ignore.strand = TRUE)
LSY4754_3A_2 <- sort(as_nt_resolved_GRanges(LSY4754_3A_2_S1_seq), ignore.strand = TRUE)

# apply Hanning smoother
n <- 51  # Hanning window size
LSY4753_3B_2_smooth <- as_smoothed_GRanges(GRanges = LSY4753_3B_2, hanning_window_size = n)
LSY4754_3A_2_smooth <- as_smoothed_GRanges(GRanges = LSY4754_3A_2, hanning_window_size = n)


# plotting ================================================================

plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/04_S1-seq_next_to_DSBs/LSY4753-3B_4754-3A"
...
for (n in 1:length(DSBs)){
  ...
  LSY4753_3B_2_roi <- subsetByOverlaps(x = LSY4753_3B_2, ranges = roi)
  LSY4754_3A_2_roi <- subsetByOverlaps(x = LSY4754_3A_2, ranges = roi)
  
  LSY4753_3B_2_smooth_roi <- subsetByOverlaps(x = LSY4753_3B_2_smooth, ranges = roi)
  LSY4754_3A_2_smooth_roi <- subsetByOverlaps(x = LSY4754_3A_2_smooth, ranges = roi)
  
  # find common ylim
  ylim <- range(c(LSY4754_3A_2_roi, LSY4753_3B_2_roi)$score)
  ylim <- c(min(ylim) - 0.05 * max(pretty(ylim)), max(pretty(ylim)))  # adjust for prettier plotting
  
  ylim_smooth <- range(c(LSY4754_3A_2_smooth_roi, LSY4753_3B_2_smooth_roi)$score)
  ...
  # make OverlayTracks
  DT_LSY4753_3B_2 <- DataTrack_raw_smooth_fw_rev(GRanges_raw = LSY4753_3B_2_roi, GRanges_smooth = LSY4753_3B_2_smooth_roi,
                                                 fw_col = fw_col, rev_col = rev_col, ylim_raw = ylim, ylim_smooth = ylim_smooth, name = "HMR-HOcs\n2 h")
  DT_LSY4754_3A_2 <- DataTrack_raw_smooth_fw_rev(GRanges_raw = LSY4754_3A_2_roi, GRanges_smooth = LSY4754_3A_2_smooth_roi,
                                                  fw_col = fw_col, rev_col = rev_col, ylim_raw = ylim, ylim_smooth = ylim_smooth, name = "hmr-HOcs\n2 h")
  
  # plot
  pdf(file = "tmp.pdf", width = 3.5, height = 2.5)
  plotTracks(trackList = list(DT_LSY4753_3B_2, DT_LSY4754_3A_2),
