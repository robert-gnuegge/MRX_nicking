...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/15_qPCR_Resection_assays/LSY4741-8C_4815-25B/Replicate_example"
moment_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/qPCR_Resection_assays/LSY4741-8C_4815-25B/Replicate_example/Moments"
raw_data_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/qPCR_Resection_assays/LSY4741-8C_4815-25B/Replicate_example/qPCR_data"
...
# set up data frame
qPCR <- data.frame(well = paste0(rep(LETTERS[9:16], rep(24, 8)), formatC(x = c(1:24), width = 2, format = "d", flag = "0")),
                   sample = rep(c("4741-8C", "4815-25B"), rep(24*4, 2)),
                   digest = rep(c(rep("none", 15), rep("RsaI", 9)), 8),
                   time = rep(rep(c(0, 1, 2, 4), rep(24, 4)), 2),
                   primers = rep(rep(c("oRG46_oRG47", "oRG54_oRG55", "oRG50_oRG51", "oRG52_oRG53", "oRG341_oRG342", "oRG54_oRG55", "oRG50_oRG51", "oRG52_oRG53"), rep(3, 8)), 8),
                   Cq = 0)
...
# check NTCs --------------------------------------------------------------
# NTCs were checked on another plate (pipetted with the same qPCR mixes) and looked good
...
for (strain in strains){
  ...
  for (cut.primers in c("oRG46_oRG47", "oRG341_oRG342")){
    # calc cut fraction
...
for (n in 1:length(strains)){
  ...
  # add HOcs cutting
  tmp <- Cut.fraction[Cut.fraction$strain == strains[n] & Cut.fraction$primers == "oRG46_oRG47", ]
  
  # time values with jitter
  t <- jitter(x = tmp$time, factor = 0.2)
  ...
  # add gSrfIcs cutting
  tmp <- Cut.fraction[Cut.fraction$strain == strains[n] & Cut.fraction$primers == "oRG341_oRG342", ]
...
for (strain in strains){
  ...
  for (resect.primers in c("oRG50_oRG51", "oRG52_oRG53")){
    ...
    tmp <- CalcResectedFraction2(resect.mock = Cq.modes[Cq.modes$sample == strain & Cq.modes$primers == resect.primers & Cq.modes$digest == "none", c("time", "mean", "sd")], 
                                 ...
                                 cut.fraction = Cut.fraction[Cut.fraction$strain == strain & Cut.fraction$primers == "oRG46_oRG47", c("time", "mean", "sd")]
    )
