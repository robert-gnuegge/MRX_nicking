...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/15_qPCR_Resection_assays/LSY4753-3B_4754-3A/Replicate_example"
moment_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/qPCR_Resection_assays/LSY4753-3B_4754-3A/Replicate_example/Moments"
raw_data_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/qPCR_Resection_assays/LSY4753-3B_4754-3A/Replicate_example/qPCR_data"
...
# define plate layout ---------------------------------------------------------
qPCR <- data.frame(well = c(paste0(rep(LETTERS[1:8], rep(18, 8)), formatC(x = 2:19, width = 2, format = "d", flag = "0")),
                            paste0(rep(LETTERS[9:16], rep(9, 8)), formatC(x = 5:13, width = 2, format = "d", flag = "0")),
                            paste0(rep(LETTERS[1:6], rep(3, 6)), formatC(x = 21:23, width = 2, format = "d", flag = "0"))),
                   sample = c(rep(c("4753-3B", "4754-3A"), rep(18 * 4, 2)), 
                              rep(c("4753-3B", "4754-3A"), rep(9 * 4, 2)), 
                              rep("NTC", 18)),
                   time = c(rep(rep(c(0, 1, 2, 4), rep(18, 4)), 2), rep(rep(c(0, 1, 2, 4), rep(9, 4)), 2), rep(0, 18)),
                   digest = c(rep(c(rep("none", 12), rep("RsaI", 6)), 8), rep(c(rep("none", 6), rep("MseI", 3)), 8), rep("none", 18)),
                   primers = c(rep(rep(c("oRG54_oRG55", "oRG46_oRG47", "oRG50_oRG51", "oRG52_oRG53", "oRG50_oRG51", "oRG52_oRG53"), rep(3, 6)), 8),
                               rep(rep(c("oRG496_oRG497", "oRG808_oRG809", "oRG808_oRG809"), rep(3, 3)), 8),
                               rep(c("oRG54_oRG55", "oRG46_oRG47", "oRG50_oRG51", "oRG52_oRG53", "oRG496_oRG497", "oRG808_oRG809"), rep(3, 6))),
                   Cq = 0)
...
for (strain in strains){
  ...
  for(cut.primers in c("oRG46_oRG47", "oRG496_oRG497")){
    # calc cut fraction
...
tmp <- Cut.fraction[Cut.fraction$primers == "oRG46_oRG47", ]
...
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/MATa_cut_fraction.pdf"))

# plot legend -----------------------------------------------------------------
LegendTxt <- c("Silenced", "Unsilenced")
MyColors <- c(JFly_colors[1:2])

pdf(file = "tmp.pdf", width=1.25, height=0.65)
par(cex = 1, mar = rep(0, 4))

plot(1, type="n", axes=FALSE, xlab="", ylab="")
legend(1, 1, pch = 20, col = MyColors, legend = LegendTxt, xjust=0.5, yjust=0.5, ncol = 1)
...
tmp <- Cut.fraction[Cut.fraction$primers == "oRG496_oRG497", ]
...
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/HMR_cut_fraction.pdf"))
...
strains <- c("4753-3B", "4754-3A")
cut.primers <- "oRG496_oRG497"
resect.primers <- "oRG808_oRG809"
...
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Resection_MATa_98_bp.pdf"))
...
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Resection_MATa_640_bp.pdf"))


# HMR -607 bp -------------------------------------------------------------
tmp <- Resected.fraction[Resected.fraction$primers == "oRG808_oRG809", ]
...
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Resection_HMR_-607_bp.pdf"))