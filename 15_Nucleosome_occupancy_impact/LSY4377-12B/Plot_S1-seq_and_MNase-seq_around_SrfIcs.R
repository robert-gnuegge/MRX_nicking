...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/13_Nucleosome_occupancy_impact/LSY4377-12B_4377-15A/S1-seq_and_MNase-seq_around_SrfIcs"
...
MNase_seq_col <- gray(level = 0.8)
MNase_seq_border_col <- NA
...
for (n in 1:length(SrfIcs)){
  ...
  DT_S1_seq_0_smooth <- DataTrack_fw_rev(GRanges = S1_seq_0_smooth, fw_col = fw_col, rev_col = rev_col, type = "l", name = "0 h", ylim = ylim_smooth)
  DT_S1_seq_1_smooth <- DataTrack_fw_rev(GRanges = S1_seq_1_smooth, fw_col = fw_col, rev_col = rev_col, type = "l", name = "1 h", ylim = ylim_smooth)
  DT_S1_seq_2_smooth <- DataTrack_fw_rev(GRanges = S1_seq_2_smooth, fw_col = fw_col, rev_col = rev_col, type = "l", name = "2 h", ylim = ylim_smooth)
  DT_S1_seq_4_smooth <- DataTrack_fw_rev(GRanges = S1_seq_4_smooth, fw_col = fw_col, rev_col = rev_col, type = "l", name = "4 h", ylim = ylim_smooth)
  ...
  # plot
  pdf(file = "tmp.pdf", width = 3.5, height = 4.5)
  plotTracks(trackList = list(OverlayTrack(trackList = list(DT_MNase_seq_0, DT_S1_seq_0_smooth)),
                              OverlayTrack(trackList = list(DT_MNase_seq_1, DT_S1_seq_1_smooth)),
                              OverlayTrack(trackList = list(DT_MNase_seq_2, DT_S1_seq_2_smooth)),
                              OverlayTrack(trackList = list(DT_MNase_seq_4, DT_S1_seq_4_smooth)), AT), 
