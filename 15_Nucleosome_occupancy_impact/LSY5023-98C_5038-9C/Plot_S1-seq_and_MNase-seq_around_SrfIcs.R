...
DT_S1_seq_pho4D_smooth <- DataTrack_fw_rev(GRanges = S1_seq_pho4D_smooth_roi, fw_col = fw_col, rev_col = rev_col, type = "l", name = "pho4Delta", ylim = ylim_smooth)
DT_S1_seq_pho4_SA_smooth <- DataTrack_fw_rev(GRanges = S1_seq_pho4_SA_smooth_roi, fw_col = fw_col, rev_col = rev_col, type = "l", name = "pho4-SA", ylim = ylim_smooth)


# MNase-seq DataTracks ----------------------------------------------------

MNase_seq_pho4D_0_roi <- subsetByOverlaps(x = MNase_seq_pho4D_0, ranges = roi) 
MNase_seq_pho4D_2_roi <- subsetByOverlaps(x = MNase_seq_pho4D_2, ranges = roi) 
MNase_seq_pho4_SA_0_roi <- subsetByOverlaps(x = MNase_seq_pho4_SA_0, ranges = roi)
MNase_seq_pho4_SA_2_roi <- subsetByOverlaps(x = MNase_seq_pho4_SA_2, ranges = roi)
ylim_MNase <- range(c(MNase_seq_pho4D_0_roi, MNase_seq_pho4D_2_roi, MNase_seq_pho4_SA_0_roi, MNase_seq_pho4_SA_2_roi)$score)
ylim_MNase <- c(min(ylim_MNase) - 0.05 * max(pretty(ylim_MNase)), max(pretty(ylim_MNase)))  # adjust for prettier plotting
DT_MNase_seq_pho4D_0 <- DataTrack(range = MNase_seq_pho4D_0_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "pho4Delta", ylim = ylim_MNase)
DT_MNase_seq_pho4D_2 <- DataTrack(range = MNase_seq_pho4D_2_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "pho4Delta", ylim = ylim_MNase)
DT_MNase_seq_pho4_SA_0 <- DataTrack(range = MNase_seq_pho4_SA_0_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "pho4-SA", ylim = ylim_MNase)
DT_MNase_seq_pho4_SA_2 <- DataTrack(range = MNase_seq_pho4_SA_2_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "pho4-SA", ylim = ylim_MNase)


# plot --------------------------------------------------------------------
pdf(file = "tmp.pdf", width = 3.5, height = 5)
  plotTracks(trackList = list(DT_MNase_seq_pho4D_0,
                              OverlayTrack(trackList = list(DT_MNase_seq_pho4D_2, DT_S1_seq_pho4D_smooth)),
                              DT_MNase_seq_pho4_SA_0,
                              OverlayTrack(trackList = list(DT_MNase_seq_pho4_SA_2, DT_S1_seq_pho4_SA_smooth)),
                              AT), 
             from = start(roi), to = end(roi), chromosome = seqnames(roi), margin = 0)
dev.off()
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/", DSB_locus, "_separate.pdf"))

# record ylims (for manual scale addition)
Y_axis_ranges <- data.frame(DSB_locus = DSB_locus, 
                            y_smooth_min = ylim_smooth[1], y_smooth_max = ylim_smooth[2],
                            y_MNase_min = ylim_MNase[1], y_MNase_max = ylim_MNase[2])

write.table(x = Y_axis_ranges, file = paste0(plot_dir, "/Y_axis_ranges_separate.txt"), row.names = FALSE)
