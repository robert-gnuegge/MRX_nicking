...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/13_Nucleosome_occupancy_impact/LSY4377-12B_4377-15A_merged/S1-seq_aligned_at_nucleosomes"
...
load(file = "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/MNase-seq/LSY4377-12B_4377-15A_merged/Nuc_centers_around_SrfIcs.RData")
...
idx <- S1$DSB_kinetics_rank < 17 & S1$dist_from_nuc > -(nuc - 0.5) * nuc_dist & S1$dist_from_nuc <= 1200
S1_agg <- aggregate(score ~ dist_from_nuc, data = S1[idx], FUN = mean)
MNase_agg <- aggregate(score ~ dist_from_nuc, data = MNase[idx], FUN = mean)

pdf(file = "tmp.pdf", width=5, height=3)
par(cex = 1, mar = c(5.1, 4.1, 4.1, 2.1) - c(1.5, 0.6, 2.7, -1.5), las = 1, tcl = -0.3, mgp = c(2.5, 0.75, 0))
plotting_function(S1_seq = S1_agg, MNase_seq = MNase_agg, nuc = nuc, 
                  S1_seq_ylim = c(0,30), MNase_seq_ylim = c(0,32), xlim = c(-(nuc - 0.5) * nuc_dist, 1200))
dev.off()
