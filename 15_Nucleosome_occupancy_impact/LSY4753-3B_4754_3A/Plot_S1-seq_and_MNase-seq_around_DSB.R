...
AT <- AnnotationTrack(range = all_features, name = NULL, featureAnnotation = "id", cex = 0.5, showFeatureId = FALSE,
...
DT_S1_seq_HMR_smooth <- DataTrack_fw_rev(GRanges = S1_seq_HMR_smooth_roi, fw_col = fw_col, rev_col = rev_col, type = "l", name = "HMR", ylim = ylim_smooth)
DT_S1_seq_hmr_smooth <- DataTrack_fw_rev(GRanges = S1_seq_hmr_smooth_roi, fw_col = fw_col, rev_col = rev_col, type = "l", name = "hmr", ylim = ylim_smooth)
...
MNase_seq_hmr_0_roi <- subsetByOverlaps(x = MNase_seq_hmr_0, ranges = roi)
MNase_seq_hmr_2_roi <- subsetByOverlaps(x = MNase_seq_hmr_2, ranges = roi)
ylim_MNase <- range(c(MNase_seq_HMR_0_roi, MNase_seq_HMR_2_roi, MNase_seq_hmr_0_roi, MNase_seq_hmr_2_roi)$score)
ylim_MNase <- c(min(ylim_MNase) - 0.05 * max(pretty(ylim_MNase)), max(pretty(ylim_MNase)))  # adjust for prettier plotting
DT_MNase_seq_HMR_0 <- DataTrack(range = MNase_seq_HMR_0_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "HMR", ylim = ylim_MNase)
DT_MNase_seq_HMR_2 <- DataTrack(range = MNase_seq_HMR_2_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "HMR", ylim = ylim_MNase)
DT_MNase_seq_hmr_0 <- DataTrack(range = MNase_seq_hmr_0_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "hmr", ylim = ylim_MNase)
DT_MNase_seq_hmr_2 <- DataTrack(range = MNase_seq_hmr_2_roi, type = "polygon", col = NA, fill.mountain = rep(MNase_seq_col, 2), name = "hmr", ylim = ylim_MNase)
...
plotTracks(trackList = list(DT_MNase_seq_HMR_0,
                            OverlayTrack(trackList = list(DT_MNase_seq_HMR_2, DT_S1_seq_HMR_smooth)),
                            DT_MNase_seq_hmr_0,
                            OverlayTrack(trackList = list(DT_MNase_seq_hmr_2, DT_S1_seq_hmr_smooth)),
                            AT), 
