# info --------------------------------------------------------------------
# purpose: generate S1-seq and RNA-seq coverage plots around genomic SrfIcs in compact form
...
S1_seq <- subsetByIntersect(subject = LSY4377_12B_2_merged_S1_seq, query = roi)

# make nt-resolved GRanges objects and sort
S1_seq <- sort(as_nt_resolved_GRanges(subsetByIntersect(subject = S1_seq, query = roi)), ignore.strand = TRUE)
...
plot_dir <- "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Figures/11_Transcription_impact/LSY4377-12B_4377-15A_merged/S1-seq_and_RNA-seq_around_SrfIcs_compact"
...
for (n in 1:length(SrfIcs)){
  ...
  DT_S1_seq_smooth <- DataTrack_fw_rev(GRanges = S1_seq_smooth, type = "l", ylim = ylim_smooth, name = "S1-seq",
                                       fw_col = fw_col, rev_col = rev_col)
  ...
  pdf(file = "tmp.pdf", width = 3.5, height = 2)
    plotTracks(trackList = list(OverlayTrack(trackList = list(DT_RNA_seq, DT_S1_seq_smooth)), AT), 
