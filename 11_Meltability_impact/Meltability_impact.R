...
load(file = "/home/robert/Research/Manuscripts/My_manuscripts/20-04-17-MRX_nicking_manuscript/Data/Melting/Melting_profiles_v2.RData")
...
plot(x = 1:70, y = rep(NA, 70), ylim = range(c(nicks$Tm_5bp, bg$Tm_5bp)),
...
points(x = 1:70, y = nicks$Tm_5bp, type = "l")
points(x = 1:70, y = bg$Tm_5bp, type = "l", lty = "dashed")

legend(x = "bottomleft", legend = c("Background", "Observed"), lty = c("dashed", "solid"), bty = "n", inset = 0.03)

dev.off()
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Melting_preference_5bp_v2.pdf"))
...
dev.off()
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Melting_preference_10bp_v2.pdf"))
...
dev.off()
GS_embed_fonts(input = "tmp.pdf", output = paste0(plot_dir, "/Melting_preference_15bp_v2.pdf"))